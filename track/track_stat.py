...
# this is specifically for the video-tracking data.
    # reason : there is an extra 'treamnet' item : DCD-HTK.
...
# capitalization matters : typing : POD_1 : is not recognzed.
...
# main contrasts, vs te reference ( DBD-HTK ).
# autoomatically calculated :
    # DBD-HTK vs DBD-Ecosol 
    # DBD-HTK vs NMP
    # DBD-HTK vs DCD-HTK
...
# DCD-HTK vs DBD-Ecosol
def build_contrast_DCD_vs_Ecosol(time_point, params_list, result):
    contrast_DCD = build_contrast_vs_baseline(time_point, "DCD-HTK", params_list, result)
    contrast_Ecosol = build_contrast_vs_baseline(time_point, "DBD-Ecosol", params_list, result)
    if contrast_DCD is None or contrast_Ecosol is None:
        return None
    return contrast_DCD - contrast_Ecosol

# DCD-HTK vs NMP
def build_contrast_DCD_vs_NMP(time_point, params_list, result):
    contrast_DCD = build_contrast_vs_baseline(time_point, "DCD-HTK", params_list, result)
    contrast_NMP = build_contrast_vs_baseline(time_point, "NMP", params_list, result)
    if contrast_DCD is None or contrast_NMP is None:
        return None
    return contrast_DCD - contrast_NMP
...
for met in data_main['metric'].unique():
    ...
    for tp in time_points:
        ...
        # main contrats versus the reference ( DBD-HTK )
        ...
        # 3. DBD-HTK vs DCD-HTK
        contrast_DCD = build_contrast_vs_baseline(tp, "DCD-HTK", params_list_metric, result_metric)
        if contrast_DCD is not None:
            test_Ecosol = result_metric.t_test(contrast_DCD)
            results_metric.append({
                "metric": met,
                "time_point": tp,
                "comparison": "DCD-HTK vs DBD-HTK",
                "contrast_estimate": test_Ecosol.effect.item(),
                "p_value": test_Ecosol.pvalue.item()
            })
        ...
        # 5. DCD-HTK vs DBD-Ecosol
        contrast_DCD_vs_Ecosol = build_contrast_DCD_vs_Ecosol(tp, params_list_metric, result_metric)
        if contrast_DCD_vs_Ecosol is not None:
            test_DCD_vs_Ecosol = result_metric.t_test(contrast_DCD_vs_Ecosol)
            results_metric.append({
                "metric": met,
                "time_point": tp,
                "comparison": "DCD-HTK vs DBD-Ecosol",
                "contrast_estimate": test_DCD_vs_Ecosol.effect.item(),
                "p_value": test_DCD_vs_Ecosol.pvalue.item()
            })


        # 6. DCD-HTK vs NMP
        contrast_DCD_vs_NMP = build_contrast_DCD_vs_NMP(tp, params_list_metric, result_metric)
        if contrast_DCD_vs_NMP is not None:
            test_DCD_vs_NMP = result_metric.t_test(contrast_DCD_vs_NMP)
            results_metric.append({
                "metric": met,
                "time_point": tp,
                "comparison": "DCD-HTK vs NMP",
                "contrast_estimate": test_DCD_vs_NMP.effect.item(),
                "p_value": test_DCD_vs_NMP.pvalue.item()
            })
...
'''
    11 treatments ( an extra : velocity_mean : instead of velocity_mean_rms )
    *
    5 time-points
    *
...
final_results_df.to_excel( r'F:\OneDrive - Uniklinik RWTH Aachen\VISION\track\stat\track_stat.xlsx' , index=False)
